---
title: コミット・プッシュ関連のツール
description: lefthook, commitlint などコミット・プッシュ前のチェックツールの概要と設定
sidebar_position: 12
---
...
本章では、本プロジェクトで導入しているコミットやプッシュの際に使用するチェックツールについて説明します。

これらのツールは、コードの品質やコミットメッセージの一貫性を保つために、開発者がコミット・プッシュ操作を行う際に自動的に実行されます。
...
- Git のフック (hook) 機能を簡単に管理し、スクリプトやコマンドをフックのタイミングで実行可能にします。
...
本プロジェクトでは、共通のコミットメッセージルールを `/shared/configs/commitlint.config.base.ts` にて管理しています。
...
- 自動チェックにより、誤ったメッセージでのコミットを防止し、チーム全体のルール遵守を促進します。
- CI/CD パイプラインと連携することで、リリースノートの自動生成などの効率化が可能になります
