// src/utils/__tests__/agLogHelpers/validation.spec.ts
// @(#) : isValidLogLevel validation function BDD tests following atsushifx-style BDD

// Copyright (c) 2025 atsushifx <http://github.com/atsushifx>
//
// This software is released under the MIT License.
// https://opensource.org/licenses/MIT

import { describe, expect, it } from 'vitest';
import { AG_LOGLEVEL } from '../../../../shared/types';
import type { AgLogLevel } from '../../../../shared/types';
import { isValidLogLevel } from '../../AgLogValidators';

describe('AgLogHelpers: Validation Functions', () => {
  describe('isValidLogLevel: LogLevel validation', () => {
    describe('正常系: Valid LogLevel inputs', () => {
      describe('Standard log levels', () => {
        it('should return true for LOG level', () => {
          expect(isValidLogLevel(AG_LOGLEVEL.LOG)).toBe(true);
        });

        it('should return true for OFF level', () => {
          expect(isValidLogLevel(AG_LOGLEVEL.OFF)).toBe(true);
        });

        it('should return true for FATAL level', () => {
          expect(isValidLogLevel(AG_LOGLEVEL.FATAL)).toBe(true);
        });

        it('should return true for ERROR level', () => {
          expect(isValidLogLevel(AG_LOGLEVEL.ERROR)).toBe(true);
        });

        it('should return true for WARN level', () => {
          expect(isValidLogLevel(AG_LOGLEVEL.WARN)).toBe(true);
        });

        it('should return true for INFO level', () => {
          expect(isValidLogLevel(AG_LOGLEVEL.INFO)).toBe(true);
        });

        it('should return true for DEBUG level', () => {
          expect(isValidLogLevel(AG_LOGLEVEL.DEBUG)).toBe(true);
        });

        it('should return true for TRACE level', () => {
          expect(isValidLogLevel(AG_LOGLEVEL.TRACE)).toBe(true);
        });

        it('should return true for VERBOSE level', () => {
          expect(isValidLogLevel(AG_LOGLEVEL.VERBOSE)).toBe(true);
        });

        it('should return true for DEFAULT level', () => {
          expect(isValidLogLevel(AG_LOGLEVEL.DEFAULT)).toBe(true);
        });
      });

      describe('Numeric literal validation', () => {
        it('should return true for numeric literals matching AG_LOGLEVEL values', () => {
          expect(isValidLogLevel(-99 as AgLogLevel)).toBe(true); // DEFAULT
          expect(isValidLogLevel(-12 as AgLogLevel)).toBe(true); // LOG
          expect(isValidLogLevel(-11 as AgLogLevel)).toBe(true); // VERBOSE
          expect(isValidLogLevel(0 as AgLogLevel)).toBe(true); // OFF
          expect(isValidLogLevel(1 as AgLogLevel)).toBe(true); // FATAL
          expect(isValidLogLevel(2 as AgLogLevel)).toBe(true); // ERROR
          expect(isValidLogLevel(3 as AgLogLevel)).toBe(true); // WARN
          expect(isValidLogLevel(4 as AgLogLevel)).toBe(true); // INFO
          expect(isValidLogLevel(5 as AgLogLevel)).toBe(true); // DEBUG
          expect(isValidLogLevel(6 as AgLogLevel)).toBe(true); // TRACE
        });
      });

      describe('Comprehensive validation', () => {
        it('should validate all AG_LOGLEVEL values using Object.values iteration', () => {
          Object.values(AG_LOGLEVEL).forEach((level) => {
            expect(isValidLogLevel(level)).toBe(true);
          });
        });

        it('should validate all AG_LOGLEVEL values using Object.entries iteration', () => {
          Object.entries(AG_LOGLEVEL).forEach(([_key, value]) => {
            expect(isValidLogLevel(value)).toBe(true);
          });
        });
      });
    });

    describe('異常系: Invalid type inputs', () => {
      describe('Null and undefined', () => {
        it('should return false for undefined', () => {
          expect(isValidLogLevel(undefined as unknown as AgLogLevel)).toBe(false);
        });

        it('should return false for null', () => {
          expect(isValidLogLevel(null as unknown as AgLogLevel)).toBe(false);
        });
      });

      describe('String values', () => {
        it('should return false for numeric string values', () => {
          expect(isValidLogLevel('0' as unknown as AgLogLevel)).toBe(false);
          expect(isValidLogLevel('1' as unknown as AgLogLevel)).toBe(false);
          expect(isValidLogLevel('4' as unknown as AgLogLevel)).toBe(false);
          expect(isValidLogLevel('-99' as unknown as AgLogLevel)).toBe(false);
          expect(isValidLogLevel('-12' as unknown as AgLogLevel)).toBe(false);
        });

        it('should return false for log level label strings', () => {
          expect(isValidLogLevel('INFO' as unknown as AgLogLevel)).toBe(false);
          expect(isValidLogLevel('DEBUG' as unknown as AgLogLevel)).toBe(false);
          expect(isValidLogLevel('ERROR' as unknown as AgLogLevel)).toBe(false);
          expect(isValidLogLevel('WARN' as unknown as AgLogLevel)).toBe(false);
          expect(isValidLogLevel('FATAL' as unknown as AgLogLevel)).toBe(false);
          expect(isValidLogLevel('OFF' as unknown as AgLogLevel)).toBe(false);
          expect(isValidLogLevel('TRACE' as unknown as AgLogLevel)).toBe(false);
          expect(isValidLogLevel('VERBOSE' as unknown as AgLogLevel)).toBe(false);
        });

        it('should return false for empty and whitespace strings', () => {
          expect(isValidLogLevel('' as unknown as AgLogLevel)).toBe(false);
          expect(isValidLogLevel(' ' as unknown as AgLogLevel)).toBe(false);
          expect(isValidLogLevel('\t' as unknown as AgLogLevel)).toBe(false);
          expect(isValidLogLevel('\n' as unknown as AgLogLevel)).toBe(false);
        });

        it('should return false for random string values', () => {
          expect(isValidLogLevel('invalid' as unknown as AgLogLevel)).toBe(false);
          expect(isValidLogLevel('random' as unknown as AgLogLevel)).toBe(false);
          expect(isValidLogLevel('test' as unknown as AgLogLevel)).toBe(false);
        });
      });

      describe('Boolean values', () => {
        it('should return false for boolean values', () => {
          expect(isValidLogLevel(true as unknown as AgLogLevel)).toBe(false);
          expect(isValidLogLevel(false as unknown as AgLogLevel)).toBe(false);
        });
      });

      describe('Object values', () => {
        it('should return false for plain objects', () => {
          expect(isValidLogLevel({} as unknown as AgLogLevel)).toBe(false);
          expect(isValidLogLevel({ level: 1 } as unknown as AgLogLevel)).toBe(false);
          expect(isValidLogLevel({ value: AG_LOGLEVEL.INFO } as unknown as AgLogLevel)).toBe(false);
        });

        it('should return false for AG_LOGLEVEL object itself', () => {
          expect(isValidLogLevel(AG_LOGLEVEL as unknown as AgLogLevel)).toBe(false);
        });

        it('should return false for complex objects', () => {
          expect(isValidLogLevel(new Date() as unknown as AgLogLevel)).toBe(false);
          expect(isValidLogLevel(/regex/ as unknown as AgLogLevel)).toBe(false);
          expect(isValidLogLevel(new Error('test') as unknown as AgLogLevel)).toBe(false);
        });
      });

      describe('Array values', () => {
        it('should return false for empty arrays', () => {
          expect(isValidLogLevel([] as unknown as AgLogLevel)).toBe(false);
        });

        it('should return false for arrays with valid log levels', () => {
          expect(isValidLogLevel([1] as unknown as AgLogLevel)).toBe(false);
          expect(isValidLogLevel([AG_LOGLEVEL.INFO] as unknown as AgLogLevel)).toBe(false);
          expect(isValidLogLevel([AG_LOGLEVEL.ERROR, AG_LOGLEVEL.WARN] as unknown as AgLogLevel)).toBe(false);
        });

        it('should return false for array-like objects', () => {
          expect(isValidLogLevel({ 0: 1, length: 1 } as unknown as AgLogLevel)).toBe(false);
          expect(isValidLogLevel({ 0: AG_LOGLEVEL.INFO, length: 1 } as unknown as AgLogLevel)).toBe(false);
        });
      });

      describe('Function values', () => {
        it('should return false for function values', () => {
          expect(isValidLogLevel((() => 1) as unknown as AgLogLevel)).toBe(false);
          expect(isValidLogLevel((() => AG_LOGLEVEL.INFO) as unknown as AgLogLevel)).toBe(false);
          expect(isValidLogLevel(isValidLogLevel as unknown as AgLogLevel)).toBe(false);
        });

        it('should return false for constructor functions', () => {
          expect(isValidLogLevel(Number as unknown as AgLogLevel)).toBe(false);
          expect(isValidLogLevel(String as unknown as AgLogLevel)).toBe(false);
          expect(isValidLogLevel(Object as unknown as AgLogLevel)).toBe(false);
        });

        it('should return false for arrow functions', () => {
          const arrowFn = (): AgLogLevel => AG_LOGLEVEL.INFO;
          expect(isValidLogLevel(arrowFn as unknown as AgLogLevel)).toBe(false);
        });

        it('should return false for async functions', () => {
          const asyncFn = async (): Promise<AgLogLevel> => AG_LOGLEVEL.INFO;
          expect(isValidLogLevel(asyncFn as unknown as AgLogLevel)).toBe(false);
        });
      });

      describe('Symbol values', () => {
        it('should return false for symbol values', () => {
          expect(isValidLogLevel(Symbol('test') as unknown as AgLogLevel)).toBe(false);
          expect(isValidLogLevel(Symbol.for('level') as unknown as AgLogLevel)).toBe(false);
          expect(isValidLogLevel(Symbol(AG_LOGLEVEL.INFO) as unknown as AgLogLevel)).toBe(false);
        });

        it('should return false for well-known symbols', () => {
          expect(isValidLogLevel(Symbol.iterator as unknown as AgLogLevel)).toBe(false);
          expect(isValidLogLevel(Symbol.toStringTag as unknown as AgLogLevel)).toBe(false);
        });
      });

      describe('BigInt values', () => {
        it('should return false for bigint values', () => {
          expect(isValidLogLevel(1n as unknown as AgLogLevel)).toBe(false);
          expect(isValidLogLevel(BigInt(4) as unknown as AgLogLevel)).toBe(false);
          expect(isValidLogLevel(BigInt(AG_LOGLEVEL.INFO) as unknown as AgLogLevel)).toBe(false);
        });
      });
    });

    describe('エッジケース: Edge case numbers', () => {
      describe('Out-of-range numbers', () => {
        it('should return false for out-of-range negative numbers', () => {
          expect(isValidLogLevel(-1 as AgLogLevel)).toBe(false);
          expect(isValidLogLevel(-50 as AgLogLevel)).toBe(false);
          expect(isValidLogLevel(-97 as AgLogLevel)).toBe(false);
          expect(isValidLogLevel(-100 as AgLogLevel)).toBe(false);
          expect(isValidLogLevel(-1000 as AgLogLevel)).toBe(false);
        });

        it('should return false for out-of-range positive numbers', () => {
          expect(isValidLogLevel(7 as AgLogLevel)).toBe(false);
          expect(isValidLogLevel(10 as AgLogLevel)).toBe(false);
          expect(isValidLogLevel(100 as AgLogLevel)).toBe(false);
          expect(isValidLogLevel(1000 as AgLogLevel)).toBe(false);
        });

        it('should return false for boundary edge values', () => {
          expect(isValidLogLevel(-101 as AgLogLevel)).toBe(false); // Just below VERBOSE
          expect(isValidLogLevel(7 as AgLogLevel)).toBe(false); // Just above TRACE
          expect(isValidLogLevel(-97 as AgLogLevel)).toBe(false); // Between LOG and VERBOSE
        });
      });

      describe('Decimal numbers', () => {
        it('should return false for decimal numbers', () => {
          expect(isValidLogLevel(4.5 as AgLogLevel)).toBe(false);
          expect(isValidLogLevel(0.1 as AgLogLevel)).toBe(false);
          expect(isValidLogLevel(-99.9 as AgLogLevel)).toBe(false);
          expect(isValidLogLevel(1.000001 as AgLogLevel)).toBe(false);
        });

        it('should return false for floating-point edge cases', () => {
          expect(isValidLogLevel(0.0000001 as AgLogLevel)).toBe(false);
          expect(isValidLogLevel(-0.0000001 as AgLogLevel)).toBe(false);
          expect(isValidLogLevel(Number.EPSILON as AgLogLevel)).toBe(false);
        });
      });

      describe('Special numeric values', () => {
        it('should return false for NaN input', () => {
          expect(isValidLogLevel(NaN as AgLogLevel)).toBe(false);
        });

        it('should return false for Infinity inputs', () => {
          expect(isValidLogLevel(Infinity as AgLogLevel)).toBe(false);
          expect(isValidLogLevel(-Infinity as AgLogLevel)).toBe(false);
        });

        it('should return false for very large numbers', () => {
          expect(isValidLogLevel(Number.MAX_VALUE as AgLogLevel)).toBe(false);
          expect(isValidLogLevel(Number.MAX_SAFE_INTEGER as AgLogLevel)).toBe(false);
          expect(isValidLogLevel(Number.MIN_SAFE_INTEGER as AgLogLevel)).toBe(false);
        });

        it('should return false for very small numbers', () => {
          expect(isValidLogLevel(Number.MIN_VALUE as AgLogLevel)).toBe(false);
          expect(isValidLogLevel(-Number.MIN_VALUE as AgLogLevel)).toBe(false);
        });
      });

      describe('Number object inputs', () => {
        it('should return false for Number object input', () => {
          expect(isValidLogLevel(new Number(1) as unknown as AgLogLevel)).toBe(false);
          expect(isValidLogLevel(new Number(AG_LOGLEVEL.INFO) as unknown as AgLogLevel)).toBe(false);
          expect(isValidLogLevel(new Number(0) as unknown as AgLogLevel)).toBe(false);
        });

        it('should return false for Number objects with special values', () => {
          expect(isValidLogLevel(new Number(NaN) as unknown as AgLogLevel)).toBe(false);
          expect(isValidLogLevel(new Number(Infinity) as unknown as AgLogLevel)).toBe(false);
          expect(isValidLogLevel(new Number(-Infinity) as unknown as AgLogLevel)).toBe(false);
        });
      });

      describe('Zero variations', () => {
        it('should return true for all zero variations (they equal OFF level)', () => {
          expect(isValidLogLevel(0 as AgLogLevel)).toBe(true); // OFF
          expect(isValidLogLevel(-0 as AgLogLevel)).toBe(true); // OFF
          expect(isValidLogLevel(0.0 as AgLogLevel)).toBe(true); // OFF
          expect(isValidLogLevel(Number('0') as AgLogLevel)).toBe(true); // OFF
        });

        it('should return false for near-zero values', () => {
          expect(isValidLogLevel(0.000001 as AgLogLevel)).toBe(false);
          expect(isValidLogLevel(-0.000001 as AgLogLevel)).toBe(false);
        });
      });
    });

    describe('境界値テスト: Boundary value tests', () => {
      it('should handle exact boundary values correctly', () => {
        // Valid boundaries
        expect(isValidLogLevel(AG_LOGLEVEL.VERBOSE)).toBe(true); // -99 (lowest valid)
        expect(isValidLogLevel(AG_LOGLEVEL.TRACE)).toBe(true); // 6 (highest valid)

        // Invalid boundaries
        expect(isValidLogLevel(-100 as AgLogLevel)).toBe(false); // One below lowest
        expect(isValidLogLevel(7 as AgLogLevel)).toBe(false); // One above highest
      });

      it('should handle gaps in the enum correctly', () => {
        // There's a gap between LOG(-12) and VERBOSE(-99)
        expect(isValidLogLevel(-12 as AgLogLevel)).toBe(true); // LOG
        expect(isValidLogLevel(-99 as AgLogLevel)).toBe(true); // VERBOSE

        // Values in the gap should be invalid
        expect(isValidLogLevel(-97 as AgLogLevel)).toBe(false);
        expect(isValidLogLevel(-50 as AgLogLevel)).toBe(false);
        expect(isValidLogLevel(-1 as AgLogLevel)).toBe(false);
      });

      it('should handle continuous range boundaries', () => {
        // OFF(0) through TRACE(6) should be continuous and valid
        for (let i = 0; i <= 6; i++) {
          expect(isValidLogLevel(i as AgLogLevel)).toBe(true);
        }

        // Just outside this range should be invalid
        expect(isValidLogLevel(-1 as AgLogLevel)).toBe(false);
        expect(isValidLogLevel(7 as AgLogLevel)).toBe(false);
      });
    });
  });
});
