...
// types
import type { AgLogMessage, AgTLogLevel } from '../../../shared/types';
...
describe('AgLoggerConfig', () => {
  ...
  it('should apply defaultLogger setting', () => {
    ...
    // Verify defaultLogger was applied
    expect(config.defaultLogger).toBe(testLogger);
  });
  ...
  it('should have setLoggerAtLevel method that can be called', () => {
    const config = new AgLoggerConfig();
    const testLogger = (_message: string): void => {/* test logger */};

    // Test that setLoggerAtLevel method exists and can be called
    expect(typeof config.setLoggerAtLevel).toBe('function');

    // Test that it can be called without throwing error
    expect(() => config.setLoggerAtLevel(AG_LOGLEVEL.ERROR, testLogger)).not.toThrow();
  });
  ...
  it('should update single logger in map via updateLoggerMap', () => {
    ...
    config.setLoggerAtLevel(AG_LOGLEVEL.ERROR, testErrorLogger);
  ...
  it('should update multiple loggers in map via updateLoggerMap', () => {
    ...
    config.setLoggerAtLevel(AG_LOGLEVEL.ERROR, testErrorLogger);
    config.setLoggerAtLevel(AG_LOGLEVEL.WARN, testWarnLogger);
    config.setLoggerAtLevel(AG_LOGLEVEL.INFO, testInfoLogger);
  ...
  it('should allow loggerMap to override defaultLogger for specified levels', () => {
    ...
    // Set a default logger - this does NOT change existing loggerMap
    config.defaultLogger = testDefaultLogger;

    // loggerMap still contains NullLogger (initialized values), not the new defaultLogger
    expect(config.getLoggerFunction(AG_LOGLEVEL.ERROR)).toBe(NullLogger);
    expect(config.getLoggerFunction(AG_LOGLEVEL.WARN)).toBe(NullLogger);
    ...
    expect(config.getLoggerFunction(AG_LOGLEVEL.ERROR)).toBe(testErrorLogger);
    // Verify other levels still use NullLogger from original loggerMap
    expect(config.getLoggerFunction(AG_LOGLEVEL.WARN)).toBe(NullLogger);
    expect(config.getLoggerFunction(AG_LOGLEVEL.INFO)).toBe(NullLogger);
  });

  it('should use defaultLogger for levels not explicitly set in loggerMap', () => {
    const config = new AgLoggerConfig();
    ...
    // Clear loggerMap entries to simulate missing entries
    config.getLoggerMap().delete(AG_LOGLEVEL.ERROR);
    config.getLoggerMap().delete(AG_LOGLEVEL.WARN);

    // When loggerMap entry is missing, defaultLogger should be used via ?? fallback
    ...
    // Levels that still have loggerMap entries should use those (NullLogger)
    expect(config.getLoggerFunction(AG_LOGLEVEL.INFO)).toBe(NullLogger);
  });

  it('should have changing defaultLogger affect levels without explicit loggerMap entries', () => {
    const config = new AgLoggerConfig();
    ...
    // Clear some loggerMap entries to test fallback behavior
    config.getLoggerMap().delete(AG_LOGLEVEL.ERROR);
    config.getLoggerMap().delete(AG_LOGLEVEL.WARN);

    // Set first defaultLogger
    config.defaultLogger = testDefaultLogger1;

    // Levels without loggerMap entries should use first defaultLogger
    ...
    // Change defaultLogger
    config.defaultLogger = testDefaultLogger2;

    // Levels without loggerMap entries should now use second defaultLogger
    ...
    // Levels with loggerMap entries should remain unchanged (NullLogger)
    expect(config.getLoggerFunction(AG_LOGLEVEL.INFO)).toBe(NullLogger);
  });
